...
def add_drug(data: DrugCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def update_drug(drug_id: int, data: DrugCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def dispense(rx_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
