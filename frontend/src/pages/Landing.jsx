...
// Place your background image at: frontend/src/assets/hero-bg.jpg
// It will automatically be used as the hero background
let heroBg = ''
try { heroBg = new URL('../assets/hero-bg.jpg', import.meta.url).href } catch {}
  ...
  return (
    <div className="font-sans text-gray-800">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-gray-600">
            {['home', 'features', 'about', 'contact', 'faq'].map(s => (
              <button key={s} onClick={() => scrollTo(s)} className="capitalize hover:text-blue-700 transition">{s === 'faq' ? 'FAQ' : s.charAt(0).toUpperCase() + s.slice(1)}</button>
            ))}
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden bg-white border-t px-6 py-4 space-y-3 text-sm font-medium text-gray-600">
            {['home', 'features', 'about', 'contact', 'faq'].map(s => (
              <button key={s} onClick={() => scrollTo(s)} className="block capitalize hover:text-blue-700">{s === 'faq' ? 'FAQ' : s.charAt(0).toUpperCase() + s.slice(1)}</button>
            ))}
      ...
      <section
        ...
        style={{
          backgroundImage: heroBg ? `url(${heroBg})` : 'linear-gradient(135deg, #1e3a8a 0%, #1d4ed8 50%, #0ea5e9 100%)',
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              ...
              <div key={title} className="bg-white rounded-2xl shadow p-6 hover:shadow-md transition">
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {team.map(({ name, role, emoji }) => (
              <div key={name} className="bg-gray-50 rounded-2xl p-6 text-center shadow-sm hover:shadow-md transition">
      ...
      <section id="contact" className="py-20 bg-gray-50">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-10">
            ...
            <form className="bg-white rounded-2xl shadow p-6 space-y-4" onSubmit={e => { e.preventDefault(); alert('Message sent! We will get back to you shortly.') }}>
              <div>
          ...
          <div className="space-y-4">
            {faqs.map((faq, i) => (
              <div key={i} className="border rounded-2xl overflow-hidden">
                ...
                {openFaq === i && (
                  <div className="px-6 pb-4 text-sm text-gray-600 bg-gray-50">{faq.a}</div>
                )}
              </div>
            ))}
          </div>
